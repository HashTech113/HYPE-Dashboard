...
def list_with_counts() -> list[dict]:
    ...
    with session_scope() as session:
        ...
        # Separate query for HR-account presence: lighter than a second join,
        # and the answer is just per-company-id boolean.
        user_company_ids = {
            int(uid)
            for (uid,) in session.execute(
                select(User.company_id).where(User.company_id.is_not(None)).distinct()
            ).all()
        }
        return [
            {
                ...
                "has_users": int(r.id) in user_company_ids,
            }
            for r in rows
        ]
