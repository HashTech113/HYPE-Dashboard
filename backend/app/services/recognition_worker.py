        ...
        * **Inference cadence (~CAPTURE_INTERVAL_SECONDS)** — actually
        ...
        while not self._stop.is_set():
            ...
            if not ok or frame is None:
                # Could be transient (key-frame gap) or terminal — don't
                # tear down for a single miss, but bail if we keep failing
                # past _READ_DEADLINE_SECONDS.
                if time.monotonic() - t0 > _READ_DEADLINE_SECONDS:
                    self.status.last_error = "RTSP read timed out"
                    return
                self._stop.wait(0.2)
                continue
